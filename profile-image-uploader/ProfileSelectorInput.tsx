...
import {
  POST_DELETE_AVATAR,
  POST_UPLOAD_AVATAR,
} from "../../api/endpoints/profile_endpoints";
import { ProfileSelectorPropsTypes } from "./types";
import { BASE_URL_SERVICES } from "../../api/callApi";

interface UploadResponse {
  data?: string;
}

const ProfileSelectorInput = ({
  ...
  return (
    <div className="max-w-sm flex flex-col mx-auto p-4 pt-0 bg-white rounded-lg">
      <div className="mb-4 text-right">تصویر پروفایل</div>
            ...
            <>
              {!imgError ? (
                <img
                  src={imageUrl}
                  alt="Uploaded"
                  className={`relative w-[180px] h-[180px] object-cover border mb-4  ${loading ? "blur" : ""} ${type === "profile" ? " rounded-full" : "rounded-[16.875px]"}`}
                ...
                <img
                  ...
                  alt="Uploaded"
                  className={`relative w-[180px] h-[180px] object-cover border mb-4 ${type === "profile" ? " rounded-full" : "rounded-[16.875px]"}`}
                />
              )}
            </>
          ) : (
            <div
              className={`w-[180px] h-[180px] mb-4 flex items-center justify-center text-gray-700 ${type === "profile" ? " rounded-full" : "rounded-[16.875px]"}`}
            >
              {type === "profile" ? (
                <svg
                  width="180"
...
export default ProfileSelectorInput;
